...
9. Detail maps show the reference place marker, the user's geolocation when permitted, and a straight line between the two points. The map always uses `fitBounds`: it fits both endpoints when user location is available and a padded viewport around the place otherwise. The distance is an approximate straight-line distance and is never presented as a driving route or road distance. The place coordinate is always described as a reference point, never as a verified campsite location.
