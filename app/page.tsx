    ...
    <main className="app-shell">
      ...
      <section className="search-card" aria-label="ค้นหาสถานที่">
        ...
        <div className="search-footer">
          ...
          <Button type="primary" size="large" onClick={search} loading={loading} className="search-button" icon={<SearchOutlined aria-hidden="true" />} iconPlacement="end">ค้นหา</Button>
        </div>
      </section>
    ...
    >
      <div className="place-art-frame">
        ...
        <div className="metric-list place-overlay" aria-label="สรุปสภาพอากาศ">
          <span className="metric metric-temperature" title="อุณหภูมิเฉลี่ย"><strong>{item.metrics?.temperatureC == null ? "—" : `${item.metrics.temperatureC.toFixed(1)}°`}</strong><span className="metric-icon metric-icon-temperature" aria-hidden="true" /></span>
          <span className="metric metric-rain" title="ฝนเฉลี่ยต่อชั่วโมง"><strong>{item.metrics?.rainMmPerHour == null ? "—" : `${item.metrics.rainMmPerHour.toFixed(2)} mm`}</strong><span className="metric-icon metric-icon-rain" aria-hidden="true" /></span>
